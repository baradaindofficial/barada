"""
Creates lib/db/admin-analytics.ts -- platform-wide analytics functions:
active learners, course popularity, completion rates per course,
estimated certificate revenue (labelled as an estimate, since no real
payments table exists -- it's issued-certificate-count x list price).

Run from repo root: py step14_add_admin_analytics_lib.py
"""
import os

FILE_PATH = "lib/db/admin-analytics.ts"
